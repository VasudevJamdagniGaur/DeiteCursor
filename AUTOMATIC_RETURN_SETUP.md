...
- App automatically detects when it's opened via deep link (`jamdagni.deite.app://signup`)
...
5. **Firebase redirects to:** `jamdagni.deite.app://signup` (deep link)
...
### Deep Link URL
Currently using: `jamdagni.deite.app://signup`
...
**IMPORTANT:** Firebase might not accept `jamdagni.deite.app://` as a direct redirect URL because OAuth requires http/https URLs.
...
3. That page redirects to `jamdagni.deite.app://signup`
...
1. **Check AndroidManifest.xml:**
   - Ensure intent-filter includes `jamdagni.deite.app` scheme
...
3. **Test Deep Link Manually:**
   ```bash
   adb shell am start -a android.intent.action.VIEW -d "jamdagni.deite.app://signup" jamdagni.deite.app
   ```
